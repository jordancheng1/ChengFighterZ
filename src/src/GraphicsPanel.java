...
public class GraphicsPanel extends JPanel implements KeyListener, MouseListener, ActionListener {
    ...
    public void paintComponent(Graphics g) {
        ...
        if (!gameStarted) {
            ...
            g.drawRect(player.getxCoord() + 20, player.getyCoord(), 120, 90);
            if (player.getAlive()) {
                ...
                for (int i = 0; i < enemies.size(); i++) {
                    ...
                    g.drawRect(enemy.getxCoord() + 30, enemy.getyCoord(), enemy.getImage().getWidth() - 50, enemy.getImage().getHeight());
        ...
        // moves left (A)
        if (pressedKeys[65] && gameStarted && player.getAlive()) {
            player.faceLeft();
            player.moveLeft();
        }
        // moves right (D)
        if (pressedKeys[68] && gameStarted && player.getAlive()) {
            player.faceRight();
            player.moveRight();
        }
    }
